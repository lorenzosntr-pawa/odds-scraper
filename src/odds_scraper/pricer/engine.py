"""
Prod Engine v1 — Java-ported pricing model.

Faithful Python port of the SourceSportradar Java service's ThreeWay1UP and
Threeway2Up calculators (com.betpawa.tools.source.sportradar.service.synthetic).

Score is always (0, 0): this module is prematch-only.

Takes pre-devigged probabilities as inputs. Devig is done at the source DB
level (sporty_outcome_*_prob columns in market_snapshots). This avoids accuracy
loss from re-devigging decimal odds.

1UP requires a first-team-to-score (FTTS) market as input. If FTTS is not
provided, 1UP is returned as deactivated (Nones) — matching the Java service's
behaviour when markets are invalid. 2UP does not depend on FTTS and is still
computed.

All coefficients are taken from FeatureProperties.java defaults — application.yml
does not override them. If you change the Java defaults, update the constants
here too.
"""
